// index.js - FINAL, COMPLETE, AND ROBUST VERSION
...
const loadDB = () => {
    try {
        ...
        console.error("Critical error loading database. Starting with an empty list.", error);
        videos = []; // Safely default to an empty array on any error
    }
};
const saveDB = () => {
    try {
...
loadDB(); // Load the database as soon as the server starts
...
app.get('/watch/:id', (req, res) => {
    ...
    if (!video) return res.status(404).send('<h1>404: Video Not Found</h1><a href="/">Go Home</a>');
...
app.post('/api/upload', upload.single('videoFile'), (req, res) => {
    ...
    try {
        ffmpeg(inputFilePath)
            .on('end', () => {
                const newVideo = { id: videos.length > 0 ? Math.max(...videos.map(v => v.id)) + 1 : 1, title: req.body.title || 'Untitled', fileName: req.file.filename, path: `/uploads/${req.file.filename}`, thumbnailPath: `/thumbnails/${thumbnailFileName}`, uploadDate: new Date() };
        ...
        console.error("Critical FFMPEG setup error:", error);
        fs.unlink(inputFilePath, () => {});
        res.status(500).json({ message: 'Server configuration error. FFMPEG might not be installed correctly.' });
    }
});
